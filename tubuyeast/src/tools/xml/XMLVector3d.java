...
/**
 * An <code>XMLMappable</code> wrapper around the {@link Vector3d} class.
 * A string representation of a Vector3d must be whitespace-delimited.
 * 
 * @author tedmunds
 */
public class XMLVector3d implements XMLMappable
{
    /**
     * The underlying "primitive" value
     */
    private Vector3d contents;

    /**
     * Constructs an <code>XMLVector3d</code> with no contents, ready to be
    ...
    /**
     * Constructs an <code>XMLVector3d</code> with the specified contents, ready
     * to be exported into an XML node.
     * 
     * @param contents the underlying "primitive" contents
     */
    public XMLVector3d(Vector3d contents)
    {
        super();
            ...
            case Node.TEXT_NODE :
                ...
                if (tokenizer.countTokens() != 3)
                {
                    throw new XMLException(
                        "Invalid Node value.  "
                            + "A Vector3d string must contain three "
                            + "whitespace-separated doubles.");
                }
                try
                {
                    this.contents = new Vector3d();
                ...
                catch (NumberFormatException nfe)
                {
                    throw new XMLException(
                        "Invalid Node value.  "
                            + "A Vector3d string must contain valid doubles.",
                        nfe);
                }
                break;
     ...
     * have been filled (by {@link #setContents(Vector3d)} or
     * {@link #loadFromXML(Node)}).
     * 
     * @return the underlying primitive value
     */
    public Vector3d getContents()
    {
        return contents;
    }
     ...
     */
    public void setContents(Vector3d contents)
    {
        this.contents = contents;
    }
